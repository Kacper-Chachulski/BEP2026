...
def create(db):
    class Participant(db.Model):
        ...
        def assign_condition(self) -> None:
            ...
            # Fully random assignment among enabled conditions
            try:
                import random
                conditions = current_app.config.get('CONDITIONS', [])
                enabled_indices = [i for i, c in enumerate(conditions) if c.get('enabled', True)]
                if enabled_indices:
                    self.condition = random.choice(enabled_indices) + 1
                    return
            except Exception:
                # If anything goes wrong, fall back to the balanced assignment below
                pass
