...
def create_app(path, config_name, debug=False, reloader_off=False):
    ...
    # If the project's app package provides an initializer to set Unity headers,
    # call it. This allows the MIME/Content-Encoding adjustments in
    # AvatarFlaskApp/app/__init__.py to be registered automatically.
    try:
        # Most BOFS projects have their package named `app` on sys.path
        from app import init_unity_response_headers
        init_unity_response_headers(app)
    except Exception:
        try:
            # Fallback to a namespaced import if the project uses AvatarFlaskApp.app
            from AvatarFlaskApp.app import init_unity_response_headers
            init_unity_response_headers(app)
        except Exception:
            # If not present, ignore silently — the MIME mappings were still added
            # at import time in the project's app/__init__.py
            pass
    ...
    with app.app_context():
        ...
        # Ensure game summary log has fields for coins passed and percent collected
        check_and_add_column('game_summary_log', 'coinsPassed', 'INTEGER', 0)
        check_and_add_column('game_summary_log', 'percentCollected', 'REAL', 0.0)
